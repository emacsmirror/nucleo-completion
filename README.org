...
is loaded.  By default, the command downloads from the release tag configured by
~nucleo-completion-module-release-tag~.  Set that option to nil to use GitHub's
latest release URL instead.
...
By default, every candidate is scored by the Rust module without an Emacs Lisp
length pre-scan.  Set ~nucleo-completion-long-candidate-threshold~ to an integer
to keep unusually long candidates from dominating scoring time: candidates
longer than the threshold are filtered but not scored by the Rust module, then
appended after scored matches in their original order.

Candidates longer than ~nucleo-completion-long-candidate-regexp-threshold~
characters skip regexp expanders such as Migemo during filtering and
highlighting.  They still use the built-in fuzzy subsequence matcher.  Set the
option to nil to apply regexp expanders to every candidate regardless of length.

Candidates longer than ~nucleo-completion-long-candidate-highlight-threshold~
characters skip match highlighting.  Set the option to nil to highlight every
candidate regardless of length.

By default, candidates are sent to the Rust module as-is.  If your completion
frontend appends disambiguation characters above Unicode's maximum codepoint,
enable ~nucleo-completion-scrub-non-unicode-candidates~ to strip those
characters before the module call and restore the original candidates
afterwards.  Keeping it disabled avoids an extra Emacs Lisp scan over every
candidate in ordinary completion tables.
